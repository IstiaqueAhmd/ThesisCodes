...
class DualStreamCNN(nn.Module):
    def __init__(self, num_classes):
        ...
        # Lightweight stream definition using depthwise separable convolutions
        def create_stream():
            return nn.Sequential(
                nn.Conv2d(1, 8, 3, padding=1),
                nn.BatchNorm2d(8),
                nn.ReLU(),
                nn.Conv2d(8, 8, 3, stride=2, padding=1, groups=8),  # Depthwise
                nn.BatchNorm2d(8),
                nn.ReLU(),
                nn.Conv2d(8, 16, 1),  # Pointwise
                nn.BatchNorm2d(16),
                nn.ReLU(),
                nn.MaxPool2d(2, 2)
            )
        ...
        self.combined = nn.Sequential(
            nn.Conv2d(32, 64, 3, padding=1),  # 16*2=32 input channels
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.MaxPool2d(2, 2),
