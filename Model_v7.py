...
class DualStreamEfficientNet(nn.Module):
    def __init__(self, num_classes):
        ...
        # Function to create EfficientNet-Lite0 stream for single-channel input
        def create_stream():
            model = timm.create_model(
                'efficientnet_lite0',
                pretrained=False,  # No pretrained weights
                num_classes=0,  # Return features before classifier
                in_chans=1,  # Single-channel input
                drop_rate=0.0,  # Disable dropout in feature extractor
        ...
        # Classifier (using original dropout rate of 0.3)
        self.classifier = nn.Sequential(
            nn.Dropout(0.5),
            nn.Linear(2560, num_classes)) # 1280 features per stream * 2 streams
...
if __name__ == '__main__':
    ...
    print(device)
    model = DualStreamEfficientNet(len(train_dataset.classes)).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), "modelv7.pth")
        else:
            patience_counter += 1
    ...
    # Final evaluation
    model.load_state_dict(torch.load("modelv7.pth"))
    model.eval()
    ...
    plt.tight_layout()
    plt.savefig("confusion_matrices_modelv7.png")
    plt.show()
